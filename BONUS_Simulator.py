...
def addf(s):
    ...
    r3=s[8:]
    # c=reg["001"]+reg["010"]
    sum = bin(int(reg[r2], 2) + int(reg[r3], 2))[2:]
    if(len(sum)>16):
        reg[r1]='0'
        reg["111"]='1000' #overflow flag
        raise Exception("Overflow in addf")
    reg[r1]=sum
...
def subf(s):
    ...
    r3=s[8:]
    print(reg[r3],reg[r2])
    if(int(reg[r3],2)>int(reg[r2],2)):
        reg[r1]='0'
        reg["111"]='1000' #overflow flag
        raise Exception("reg3 > reg2 in subf")
    sub = bin(int(reg[r2], 2) - int(reg[r3], 2))[3:]
    # print(sub)
    reg[r1]=sub
...
def addi(s):
    r1=s[1:4]
    imm=s[4:]
    reg[r1]=bin(int(reg[r1],2)+int(imm,2))[2:]
...
opcodes = {
    ...
    "11010":hlt,"10000":addf,"10001":subf,"10000":addf,"10001":subf,"10010":movf,"10011":addi,"10100":subi,"10101":muli,"10110":divi,"10111":pow
}
